...
grades ──── users.grade (문자열, grades.name)
departments ──── users.dept (문자열, departments.name)
...
| name | String | 이름 |
| dept | String | 부서명 (`departments.name`, 드롭다운 선택) |
| grade | String | 직급명 (`grades.name`, 드롭다운 선택) |
...
| level | Integer | 순서 (ORM `sort_order`, **작을수록 높은 직급**). 결재는 level 큰 값부터 |
| is_active | Boolean | 활성 여부 |
...
|------|------|------|
| id | PK | |
| name | String | 부서명 (관리팀, 측정팀1...) |
| sort_order | Integer | 드롭다운·목록 표시 순서 |
| is_active | Boolean | 활성 여부 |
...
| 컬럼 | 타입 | 설명 |
|------|------|------|
| id | PK | 1=원주본사(WJ), 2=제천지사(JC) |
| name | String | 표시명 |
| code | String UNIQUE | 문서번호·NAS 경로용 |
| is_headquarters | Boolean | 본사 여부 |

**결재 문서 지사 정책** (`app/branch_scope.py`, `app/doc_requirements.py`):

- **저장:** 모든 `documents` 행은 상신자 `users.branch_id` (업무일지·출장복명서·휴가·일반 등 동일).
- **조회:** `visible_document_ids` = 조회 지사(`view_branch_id` 쿠키/쿼리, 기본 소속) + 결재선 타지사 + 본인 작성.
- **전환:** `is_admin` 또는 **원주본사** + 「대표」 이상 — 헤더 조회. **제천 대표는 전환 불가.** 통합 목록 없음.
- **업무일지:** 결재선 없음, 저장 즉시 `APPROVED_FINAL`/`RECORD`.
- **통합:** 게시판·조직도. **분리:** 품질 NAS, 회계, 지역·수금, 문서번호 접두 지사코드.

#### regions (지역 마스터) — Phase 5 + 7
| 컬럼 | 타입 | 설명 |
|------|------|------|
| id | PK | |
| name | String(50) | 지사 내 UNIQUE (`branch_id`, `name`) |
| branch_id | FK → branches | 출장·수금 지역은 **소속 지사**만 |

#### collections (수금) — Phase 5 + 7
| 컬럼 | 타입 | 설명 |
|------|------|------|
| id | PK | |
| company_name | String | 업체명 |
| region_name | String | 동일 지사 `regions.name` 과 매칭 |
| amount | Integer | 수금액 |
| collection_date | String(10) | YYYY-MM-DD |
| note | Text | 비고 |
| branch_id | FK → branches | 수금 등록 지사 |
...
#### regions / collections (Phase 5·7)
지역·수금은 **지사별** 관리. 시드: 원주·제천·단양 × 지사 1·2.
회계 청구는 `documents.branch_id` 기준 최종 승인 출장복명서 라인만 집계.
...
| 관리 | `/admin/users`, `/admin/depts`, `/admin/grades` | **관리자** (`require_admin`) |
| 내정보 | `/me` (GET/POST) | 로그인 |
...
| 구분 | 상태 |
|------|------|
| GET `/admin`, `/admin/users`, `/admin/depts`, `/admin/grades`, `/admin/users/{id}/edit` | ✅ |
| POST 사용자 추가·CSV·수정·비활성·PW초기화 | ✅ |
| POST 부서·직급 추가, ▲▼ 순서, 비활성·복구 | ✅ |
| POST/GET `/me` | ✅ |

- 부서·직급은 `users.dept` / `users.grade` 문자열에 **마스터 이름** 저장. 추가는 관리 화면에서만.
- 비활성화 시 해당 이름을 쓰던 사용자 필드 초기화.
- 결재: `grades.level` 순, 업무일지 `GRADE_TIER`, 출장복명서 `TRIP_REPORT_APPROVAL_DEPTS`.
...
**지역 정규화:** 출장은 `regions.id`(FK, 동일 `branch_id`). 수금은 `collections.region_name` = 해당 지사 `regions.name`.

**Phase 7 지사 격리:** 청구 = `Document.branch_id`, 수금 = `Collection.branch_id`. 회계·문서 목록은 `can_switch_branch_view`(관리자 또는 대표 이상)가 `?branch_id=` / 헤더 조회로 지사 전환. 일반 사용자는 소속 지사만.
