...
> 초기 버그 수정부터 Phase 1~7 확장까지 전체 변경 사항을 기록합니다.
...
## Phase 7 — 멀티 지사 (2026-05-21)

| 항목 | 내용 |
|------|------|
| `branches`, `branch_scope.py` | 원주(WJ)·제천(JC), 문서·회계·품질 지사 격리 |
| 결재 문서 | 상신자 `branch_id`로 **전 유형** 분리 (업무일지·출장복명서 포함) |
| 조회 | 관리자·**원주본사** 대표 이상만 지사 전환(제천 대표 제외) |
| 업무일지 | 결재선 없이 `RECORD` 즉시 완료 |
| 출장 작성 | 부서 접두사 `관리,측정` (관리팀·측정팀) |
| 가시성 | 조회 지사 + 결재선 타지사 + 본인 작성 |
| `doc_requirements.py` | 업무일지 의무·출장 작성 부서 제한 |
| 통합 | 게시판·조직도 / 분리 — 회계·NAS·지역 마스터 |
...
| 2026-05-21 | **Phase 7** — 지사별 문서 등록·조회 권한(원주 한정 아님, 통합 목록 없음) README·docs·`.env.example` 반영 |
